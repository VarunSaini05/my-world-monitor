...
The Docker build enables private self-hosted owner mode by default. Built-in
panel and local API subscription gates are disabled for the instance you
operate. This does not bypass third-party provider licensing or usage limits:
feeds that require an external key still need a legitimate key, and paid-only
providers should be replaced with an open source where possible.
