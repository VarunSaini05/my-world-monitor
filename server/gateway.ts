...
export function createDomainGateway(
  ...
  return async function handler(originalRequest: Request, ctx?: GatewayCtx): Promise<Response> {
    ...
    const selfHostedFullAccess = isSelfHostedFullAccess();
    ...
    const isTierGated = !selfHostedFullAccess && !internalMcpVerified && !isPublicNoAuthRpc && !seedRefreshVerified && !relayWarmPingVerified && getRequiredTier(pathname) !== null;
    const needsLegacyProBearerGate = !selfHostedFullAccess && !internalMcpVerified && !isPublicNoAuthRpc && PREMIUM_RPC_PATHS.has(pathname) && !isTierGated;
    ...
    let keyCheck: { valid: boolean; required: boolean; error?: string; kind?: 'enterprise' | 'session' | 'user' } = selfHostedFullAccess || internalMcpVerified || isPublicNoAuthRpc || seedRefreshVerified || relayWarmPingVerified
